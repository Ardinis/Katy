...
void NtSection::SuspendAllExceptCurrentThread(HANDLE process)
{
    ...
    do
    {
        if (threadEntry.th32OwnerProcessID == GetProcessId(process) && threadEntry.th32ThreadID != GetCurrentThreadId())
        {
...
void NtSection::ResumeProcess(HANDLE process)
{
    HANDLE hThreadSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPTHREAD, 0);
...
std::unique_ptr<NtSection> NtSection::CopyCodeSection(HANDLE process)
{
    ...
    SuspendAllExceptCurrentThread(process);
    std::shared_ptr<void> threads(nullptr, [process](void*)
    {
